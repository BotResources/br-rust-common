...
use support::{
    RecordingStager, drop_published_group, drop_published_user, exclusive_projection, fabric,
    foreign_ref, group, infra, isolated_pool, manifest, members, staged_keys, user, user_row,
};
...
#[tokio::test]
#[ignore = "requires NATS_URL and TEST_DATABASE_URL pointing at a real broker and Postgres"]
async fn an_orphan_delete_stages_one_impact_for_the_removed_user() {
    let (nats, database) = infra();
    let exclusive = exclusive_projection(&database).await;
    let fabric = fabric(&nats).await;
    let pool = isolated_pool(&database).await;
    ...
    assert_eq!(
        staged_keys(&pool, user_id).await,
        2,
        "a removal is a change and stages its own impact"
    );
    assert_eq!(progress.borrow().changes, after_first + 1);
    ...
    assert_eq!(
        staged_keys(&pool, user_id).await,
        2,
        "the user is gone from both sides: nothing left to retract"
    );
    assert_eq!(progress.borrow().changes, after_first + 1);
